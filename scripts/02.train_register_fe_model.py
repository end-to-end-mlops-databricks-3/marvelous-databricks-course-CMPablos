...
spark = SparkSession.builder.getOrCreate()
test_set = spark.table(f"{config.catalog_name}.{config.schema_name}.hotel_reservations_test_set").limit(100)
# Drop feature lookup columns and target
test_set = test_set.drop("lead_time", "no_of_special_requests", "arrival_year")
